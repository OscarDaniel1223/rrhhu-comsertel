...
- [x] **Tarea 31 (Frontend y backend):** Diseñar e implementar el módulo de "Programación de Aguinaldo". Esto incluye agregar la columna `fecha_aguinaldo` en la tabla `empleados`, crear una nueva interfaz gráfica para calendarizar la fecha de pago del aguinaldo para cada empleado, y modificar el motor de nómina para automatizar el cálculo del aguinaldo cuando la fecha programada caiga en el rango de fechas de la planilla. (Completado)
- [x] **Tarea 32 (Frontend y backend):** Modificar la interfaz de "Generar Planilla" para extraer, mostrar de manera informativa y verificar las fechas programadas de pago de aguinaldo para los colaboradores de forma automática al seleccionar el mes a procesar, asegurando la consistencia y correcta visualización del aguinaldo en la planilla consolidada. (Completado)
- [x] **Tarea 33 (Frontend):** Mover la tabla "Matriz Planilla" de la UI "Formato Planilla Consolidada" hacia la UI "Detalle de Empleados en Planilla" (acceso mediante "Ver detalle" en Historial de Planillas) para consolidar los reportes detallados, y remover el elemento de menú de formato obsoleto para simplificar la navegación de la plataforma. (Completado)
- [x] **Tarea 34 (Frontend y backend):** Diseñar e implementar el modulo de "Gestion de Novedades y Horas Extras" de forma diaria e historico mensual. Esto incluye crear la tabla novedades_empleados en la base de datos MariaDB para almacenar viaticos, beneficios, horas extras diurnas y nocturnas por empleado y fecha diaria. Desarrollar las APIs del backend con filtros por dia y rango de fechas acumulado, y crear una interfaz grafica con pestañas en React para registrar masivamente novedades por dia y visualizar el historial detallado de sumas acumuladas por colaborador filtrando por mes. Modificar la UI de "Generar Planilla" para eliminar los inputs manuales y cargar en automatico los acumulados de novedades en el periodo. (Completado)
- [x] **Tarea 35 (Frontend):** Modificar la barra de navegacion de la izquierda (Sidebar) para que el usuario pueda ocultarla o colapsarla en pantallas de escritorio mediante un boton flotante, y mostrarla nuevamente a traves de un boton en la interfaz principal. (Completado)